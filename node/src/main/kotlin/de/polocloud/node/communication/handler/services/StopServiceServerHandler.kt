...
/**
 * Handles a peer's `StopService` request — the cross-node counterpart of the CLI's
 * `service <name> shutdown`, used when the service is running on a different node than
 * the one the CLI command was issued on. See [de.polocloud.node.terminal.impl.ServiceCommand].
 */
